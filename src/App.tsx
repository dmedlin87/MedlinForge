        ...
        <main className="flex-1 space-y-6" aria-label="Main content">
          ...
          {state && (
            <>
              ...
              {screen === 'addons' && <AddonsPanel addons={filteredAddons} allAddonsCount={state.addons.length} search={addonSearch} setSearch={setAddonSearch} onInstall={previewInstall} onChannel={previewChannel} onRemove={previewRemove} onIsolate={(addonId) => state.activeProfileId && void previewSync(state.activeProfileId, false, addonId)} />}
              ...
              {screen === 'settings' && <SettingsPanel draft={settingsDraft} setDraft={setSettingsDraft} detectedPaths={detectedPaths} onDetect={() => void detectPaths()} onSave={() => void saveSettings()} />}
...
function Dashboard({ state }: { state: ScanStateResponse }) {
  return (
    <div className="grid gap-6 xl:grid-cols-[1.3fr_1fr]">
      <Panel title="Issues" subtitle="Blockers and warnings from the active profile.">
...
function AddonsPanel({ addons, allAddonsCount, search, setSearch, onInstall, onChannel, onRemove, onIsolate }: { addons: AddonRecord[]; allAddonsCount: number; search: string; setSearch: (value: string) => void; onInstall: (addonId: string) => void; onChannel: (addonId: string, channel: Channel) => void; onRemove: (addonId: string) => void; onIsolate: (addonId: string) => void }) {
