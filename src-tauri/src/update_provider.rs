...
use crate::{
    models::{RemoteProductType, Settings, UpdateChannel},
    service::{ServiceError, ServiceResult},
};

const DEFAULT_MANIFEST_BASE_URL: &str = env!("BRONZEFORGE_UPDATE_MANIFEST_BASE_URL");
...
impl UpdateProvider {
    ...
    pub fn resolve_manifest_url(&self, settings: &Settings) -> ServiceResult<String> {
        let base = settings
        ...
        let base_url = Url::parse(normalized_base.trim())
            .map_err(|error| ServiceError::Message(format!("Invalid update manifest base URL: {error}")))?;
        if base_url.scheme() != "https" && !(settings.dev_mode_enabled && base_url.scheme() == "http")
        {
            return Err(ServiceError::Message(
                "Update manifest base URL must use HTTPS unless dev mode override is enabled"
                    .to_string(),
            ));
        }
        ...
        Ok(base_url
            ...
            .map_err(|error| ServiceError::Message(format!("Invalid update manifest URL: {error}")))?
            .to_string())
    }

    pub fn fetch_manifest(&self, settings: &Settings) -> ServiceResult<(String, UpdateManifest)> {
        let manifest_url = self.resolve_manifest_url(settings)?;
        let response = self
            .client
            .get(&manifest_url)
            .send()
            .map_err(|error| ServiceError::Message(format!("Failed to fetch update manifest: {error}")))?;
        ...
        let body = response
            .text()
            .map_err(|error| ServiceError::Message(format!("Failed to read update manifest: {error}")))?;
        let manifest: UpdateManifest = serde_json::from_str(&body)?;
        self.validate_manifest(&manifest_url, &manifest, settings.dev_mode_enabled)?;
    ...
    pub fn download_verified_asset(
        ...
        let mut response = self
            ...
            .send()
            .map_err(|error| ServiceError::Message(format!("Failed to download {}: {error}", product.id)))?;
        if !response.status().is_success() {
            return Err(ServiceError::Message(format!(
        ...
        loop {
            let read = response
                .read(&mut buffer)
                .map_err(|error| ServiceError::Message(format!("Failed to read {}: {error}", product.id)))?;
            if read == 0 {
                break;
            }
    ...
    pub fn is_remote_newer(remote_version: &str, current_version: Option<&str>) -> bool {
        ...
        match (parse_version(remote_version), parse_version(current_version)) {
    ...
    pub fn manager_version_satisfies(minimum_version: Option<&str>) -> bool {
        ...
        match (parse_version(env!("CARGO_PKG_VERSION")), parse_version(minimum_version)) {
    ...
    fn validate_manifest(
        ...
        validate_url(manifest_url, allow_http_override)?;
        ...
        if manifest.publisher != DEFAULT_PUBLISHER {
            return Err(ServiceError::Message(format!(
                "Unexpected manifest publisher '{}'",
                manifest.publisher
            )));
        }
        for (product_id, product) in &manifest.products {
            if product.id != *product_id {
                return Err(ServiceError::Message(format!(
                    "Manifest product key '{}' does not match payload id '{}'",
                    product_id, product.id
                )));
            }
            if product.channel != manifest.channel {
                return Err(ServiceError::Message(format!(
                    "Manifest product '{}' channel does not match manifest channel",
                    product.id
                )));
            }
    ...
    fn validate_product(
        ...
        if !product.sha256.chars().all(|value| value.is_ascii_hexdigit()) || product.sha256.len() != 64
        {
            return Err(ServiceError::Message(format!(
...
#[cfg(test)]
mod tests {
    use super::{RemoteManifestProduct, UpdateProvider};
    use crate::models::{RemoteProductType, UpdateChannel};
    ...
    fn product(version: &str) -> RemoteManifestProduct {
        RemoteManifestProduct {
            ...
            release_url: "https://github.com/dmedlin87/MedlinForge/releases/tag/v1.2.3"
                .to_string(),
            package_url:
    ...
    fn remote_version_comparison_prefers_semver() {
        ...
        assert!(UpdateProvider::is_remote_newer("1.3.0-beta.1", Some("1.2.9")));
    }
